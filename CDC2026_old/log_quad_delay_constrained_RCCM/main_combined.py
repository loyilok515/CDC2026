...
alpha_lb = 0.5  # Lower bound of the alpha parameter in RCCM

# Configuration variables
task = 'quad_delay'  # Name of the model
log = 'log_quad_delay_constrained_RCCM'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, _lambda, verbose=False, acc=False, detach=False):
    ...
    loss_RCCM2 = loss_pos_matrix_eigen_values(RCCM_2 - epsilon * torch.eye(RCCM_2.shape[-1]).unsqueeze(0).type(x.type()))
    loss_alpha = torch.relu(_alpha - alpha_lb)

    loss = loss_CCM + loss_w_ub + loss_C1 + loss_C2 + loss_RCCM1 + loss_RCCM2 + loss_alpha

    if verbose:
        print("loss_CCM: %.6f, loss_w_ub: %.6f, loss_C1: %.6f, loss_C2: %.6f, loss_RCCM1: %.6f, loss_RCCM2: %.6f, loss_alpha: %.6f"%(loss_CCM.item(), loss_w_ub.item(), loss_C1.item(), loss_C2.item(), loss_RCCM1.item(), loss_RCCM2.item(), loss_alpha.item()))
...
def trainval(X, bs=bs, train=True, _lambda=_lambda, acc=False, detach=False): # trainval a set of x
    ...
    for b in _iter:
        ...
        loss, p1, p2, l3, r4, r5 = forward(x, xref, uref, w, _lambda=_lambda, verbose=True if not train else False, acc=acc, detach=detach)
        ...
        if acc:
            ...
            total_r4 += r4.sum()
            total_r5 += r5.sum()
    return total_loss / len(X), total_p1 / len(X), total_p2 / len(X), total_l3/ len(X), total_r4 / len(X), total_r5 / len(X)
...
for epoch in range(0, epochs):
    ...
    loss, _, _, _, _, _ = trainval(X_tr, train=True, _lambda=_lambda, acc=False, detach=True if epoch < detach_epoch else False)
    print("Training loss: ", loss)
    loss, p1, p2, l3, r4, r5 = trainval(X_te, train=False, _lambda=_lambda, acc=True, detach=False)
    print("Epoch %d: Testing loss/p1/p2/l3/r4/r5: "%epoch, loss, p1, p2, l3, r4, r5)  # Ideal: 0 / 1 / 1 / 0 / 1 / 1
