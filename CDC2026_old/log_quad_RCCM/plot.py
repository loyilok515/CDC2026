...
from utils_trajectory_generation import hover_trajectory_generator, circular_trajectory_generator
import time
...
plot_types = ['2D', '3D', 'time', 'attitude', 'error', 'control', 'control_error', 'disturbance_est_err'] # specify which types of plots to generate; options: '2D', '3D', 'time', 'attitude', 'error', 'control', 'control_error', 'disturbance_err'
plot_dims_dict = {'2D': [0, 1], '3D': [0, 1, 2], 'time': [0, 1, 2], 'attitude': [0, 1, 2], \
                  'error': [0, 1, 2, 3], 'control': [0, 1, 2, 3], 'control_error': [0, 1, 2, 3], 'disturbance_est_err': [0, 1, 2]} # specify which state dimensions to plot
...
task = 'quad'
log = 'log_quad_constrained_RCCM2' # You'll need to set this to the path of your log model
trajectory_generator = hover_trajectory_generator
...
simulate = False  # Whether to run the simulation and save data to csv; if False, will load from csv if it exists
UDE_activated = True  # UDE activation flag
...
controller = get_controller_wrapper(log + '/controller_best.pth.tar')
...
if __name__ == '__main__':
    ...
    if simulate or (not os.path.exists(csv_path)):
        ...
        for _ in range(nTraj):
            ...
            xstar, x, x_att, ustar, u, d_hat_err = RK4(controller, trajectory_generator, UDE_activated, f, B, B_w, x_0, time_bound, time_step, w_MIN, w_MAX)
