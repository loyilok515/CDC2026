...
def RK4(controller, trajectory_generator, UDE_activated, f, B, B_w, xinit, t_max, dt, w_MIN, w_MAX):
    ...
    for i in range(len(t)):
        ...
        tau_a = 0.2 # time constant for acceleration
        tau_omega = 0.05 # time constant for omega
        delay_matrix = np.diag([1/tau_a, 1/tau_omega, 1/tau_omega, 1/tau_omega])
        if i>0:
            u_dot = delay_matrix @ (-u[-1] + ui_desired)
            ui = u[-1] + u_dot * dt
        else:
            ui = ui_desired 
