...
def RK4(controller, trajectory_generator, UDE_activated, f, B, B_w, xinit, t_max, dt, w_MIN, w_MAX):
    ...
    for i in range(len(t)):
        ...
        # Actuator dynamics
        # Force dynamics
        tau_a = 1.0 # time constant for acceleration   ###Temp
        ...
        # Torque dynamics
        torque_dynamics_activated = True
        ...
        vnext = rk4_step(x = xcurr, dt = dt, func = system_dynamics, u = ui, w = noise, f = f, B = B, B_w = B_w)[0:7]  # [0:6]
    ...
    return trace_star, trace, trace_att, u_star, u_CCM, u, d_hat_err_trace
...
### Temp
# def system_dynamics_noisy(x, u, w, f, B, B_w, d_control, d_state_est):
#     return f(x) + B(x).dot(u.reshape(-1,1)+d_control.reshape(-1,1)) + B_w(x).dot(w.reshape(-1,1)) + d_state_est.reshape(-1,1)
