...
class U_FUNC(nn.Module):
    ...
    def forward(self, x, xstar, uref):
        ...
        u_raw[:, 1:4, :] = torch.zeros(bs, 3, 1).type(x.type()) # No CCM required in controlling d
        
        # # Apply control bounds with smoother saturation
        # bounds = torch.tensor([T_bound, d_bound, d_bound, d_bound, omega_bound, omega_bound, omega_bound])
        # bounds = bounds.type(x.type()).view(1, -1, 1).expand(bs, -1, -1)
