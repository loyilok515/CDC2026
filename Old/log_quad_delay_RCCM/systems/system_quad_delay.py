...
def S_func(x):
    ...
    # Extract states
    p1, p2, p3, v1, v2, v3, a, r1, r2, r3, r4, r5, r6, r7, r8, r9 = [x[:, i, 0] for i in range(num_dim_x)]
    ...
    Sa = torch.eye(1).type(x.type()).repeat(bs, 1, 1)
...
def f_func(x):
    ...
    f[:, 3:6, 0] = g_I + torch.bmm(R, torch.bmm(e3, a)).reshape(bs, 3)
    f[:, 6:7, 0] = -1/tau * a.reshape(bs, 1)
...
def B_func(x):
    ...
    B[:, 6, 0] = 1/tau
    B[:, 7:16, 1:4] = torch.cat([skew(r_row1), skew(r_row2), skew(r_row3)], dim=1)  # (bs, 9, 3)
...
def E_func(x):
    ...
    E[:, 6, 0] = 1/tau
    E[:, 7:10, 1:4] = np.sqrt(2) * torch.eye(3).type(x.type()).repeat(bs, 1, 1)
    return E
...
def Ebot_func(x):
    ...
    Ebot[:, 0:6, 0:6] = torch.eye(6).type(x.type()).repeat(bs, 1, 1)
    return Ebot
