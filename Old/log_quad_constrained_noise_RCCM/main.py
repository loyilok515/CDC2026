...
task = 'quad_noise'  # Name of the model
log = 'log_quad_constrained_noise_RCCM'  # Path to a directory for storing the training log
...
sys.stdout = Tee(log + "/training_log.txt")
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    loss_C2w = 1. * sum([1.*(C2w**2).reshape(bs,-1).sum(dim=1).mean() for C2w in C2ws])
    ...
    if epoch < RCCM_start_epoch: 
        ...
        loss = loss_CCM + loss_w_ub + loss_C1 + loss_C2 + loss_RCCM1 + loss_RCCM2 + loss_alpha + loss_C2w

    if verbose:
        print("loss_CCM: %.6f, loss_w_ub: %.6f, loss_C1: %.6f, loss_C2: %.6f, loss_C2w: %.6f, loss_RCCM1: %.6f, loss_RCCM2: %.6f, loss_alpha: %.6f"%(loss_CCM.item(), loss_w_ub.item(), loss_C1.item(), loss_C2.item(), loss_C2w.item(), loss_RCCM1.item(), loss_RCCM2.item(), loss_alpha.item()))
