...
def get_model(num_dim_x, num_dim_manifold, num_dim_control, w_lb, use_cuda=False):
    ...
    param_alpha_init = torch.log(torch.exp(torch.tensor(1.4, device=device)) - 1.0)  # Inverse of softplus
    param_miu_init = torch.log(torch.exp(torch.tensor(1.2, device=device)) - 1.0)
