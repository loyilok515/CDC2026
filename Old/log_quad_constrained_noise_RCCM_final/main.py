...
log = 'log_quad_constrained_noise_RCCM_final' #'log_quad_RCCM_torque'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    loss_alpha = 0.1 * torch.relu(_alpha - alpha_lb)
...
def trainval(X, bs=bs, train=True, acc=False, detach=False): # trainval a set of x
    ...
    for b in _iter:
        ...
        if train:
            ...
            # Gradient logging
            stats_u_w1 = compute_grad_stats(model_u_w1.named_parameters())
            stats_u_w2 = compute_grad_stats(model_u_w2.named_parameters())
            stats_W    = compute_grad_stats(model_W.named_parameters())
            print_stats(stats_u_w1, "u_w1")
            print_stats(stats_u_w2, "u_w2")
            print_stats(stats_W, "W")
...
def print_stats(stats, title):
    ...
    for k, v in stats.items():
        print(f"{k}: grad={v['grad_norm']:.2e}, ratio={v['ratio']:.2e}")
