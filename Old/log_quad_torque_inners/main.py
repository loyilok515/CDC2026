...
log = 'log_quad_torque_inners' #'log_quad_RCCM_torque'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    loss_C2 = 4. * sum([1.*(C2_inner**2).reshape(bs,-1).sum(dim=1).mean() for C2_inner in C2_inners])
