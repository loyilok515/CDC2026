...
def hover_trajectory_generator(t, dist_est = np.array([[0.], [0.], [0.]])):
    ...
    v_B = R @ v  # Velocity in body frame
...
def circular_delayed_trajectory_generator(t, dist_est = np.array([[0.], [0.], [0.]]), r_c = 1.5, omega = 0.2, tau = 0.1):
    ...
    py = r_c * np.sin(omega * t)
    pz = 1
    p = np.array([px, py, pz])  # Position in inertial frame
    vx = -omega*r_c * np.sin(omega * t)
    vy = omega*r_c * np.cos(omega * t)
    vz = 0
    v = np.array([vx, vy, vz])  # Velocity in inertial frame
    ...
    T_comp = np.sqrt( (d_hat[0] + omega**2*r_c*np.cos(omega*t))**2 + (d_hat[1] + omega**2*r_c*np.sin(omega*t))**2 + (d_hat[2]-g)**2 )
    a = np.array([T_comp])
    xstar_t = np.concatenate([p, v, a, r]).reshape(-1, 1)

    # uref
    T_dot = omega**3*r_c*(d_hat[1]*np.cos(omega*t)-d_hat[0]*np.sin(omega*t))/s3
    T_cmd = tau*T_dot + T_comp
    p_comp = -(omega**3*r_c*np.cos(omega*t)*(d_hat[2]-g))/(T_comp*np.sqrt( (d_hat[1] + omega**2*r_c*np.sin(omega*t))**2 + (d_hat[2]-g)**2 ))
    q_comp = -p_comp * ( (d_hat[1]**2 + (d_hat[2]-g)**2 + omega**4*r_c**2)*np.sin(omega*t) + (d_hat[1] + d_hat[1]*np.sin(omega*t)**2 + d_hat[0]*np.sin(omega*t)*np.cos(omega*t))*omega**2*r_c + d_hat[0]*d_hat[1]*np.cos(omega*t) ) / (T_comp * np.cos(omega*t) * (d_hat[2]-g))
    r_comp = 0
    ustar_t = np.array([T_cmd, p_comp, q_comp, r_comp]).reshape(-1,1)
...
# xstar_prev = np.zeros([num_dim_x,1])
# for i in range(len(t)):
#     _t = dt*i  # Current time
#     trajectory_generator = circular_delayed_trajectory_generator
#     dist_est = np.array([0.2, -0.3, 0.1]).reshape(-1,1)
#     xstar_t, ustar_t = trajectory_generator(_t, dist_est)
#     xstar_dot = (xstar_t - xstar_prev)/dt
#     xstar_dot_sys = f(xstar_t)+B(xstar_t)@ustar_t+B_w(xstar_t)@dist_est
#     x_diff = xstar_dot - xstar_dot_sys
#     if np.linalg.norm(x_diff) > 1e-5 and i>0:
#         # print(x_diff)
#     xstar_prev = xstar_t
