...
def f_func(x):
    ...
    omega = torch.stack([omegea1, omega2, omega3], dim=1).reshape(bs, 3, 1)  # (bs, 3, 1)
    r = torch.stack([r1, r2, r3, r4, r5, r6, r7, r8, r9], dim=1).reshape(bs, 9, 1)  # (bs, 9, 1)
    ...
    # Angular velocity
    cross_term = torch.bmm(skew(omega.reshape(bs, 3)), torch.bmm(J.repeat(bs, 1, 1), omega))
    ...
    f[:, 3:6, 0] = g_I
    f[:, 6:9, 0] = torch.bmm(J_inv, cross_term)
    f[:, 9:18] = r_dot
