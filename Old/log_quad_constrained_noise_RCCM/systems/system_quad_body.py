...
def f_func(x):
    ...
    v_b = torch.stack([v1, v2, v3], dim=1).reshape(bs, 3, 1)  # (bs, 3, 1)  # Body velocities
    r = torch.stack([r1, r2, r3, r4, r5, r6, r7, r8, r9], dim=1).reshape(bs, 9, 1)  # (bs, 9, 1)
    ...
    # Gravity
    g = 9.81
    g_I = torch.tensor([0, 0, -g]).type(x.type()).repeat(bs, 1).reshape(bs, 3, 1) # (bs, 3, 1)

    f = torch.zeros(bs, num_dim_x, 1).type(x.type())
    f[:, 0:3, 0] = torch.bmm(R, v_b).reshape(bs, 3)  
    f[:, 3:6, 0] = torch.bmm(R.transpose(1,2), g_I).reshape(bs, 3)
    return f
...
def B_func(x):
    ...
    p1, p2, p3, v1, v2, v3, r1, r2, r3, r4, r5, r6, r7, r8, r9 = [x[:, i, 0] for i in range(num_dim_x)]
    p = torch.stack([p1, p2, p3], dim=1).reshape(bs, 3, 1)  # (bs, 3, 1)
    v_b = torch.stack([v1, v2, v3], dim=1).reshape(bs, 3, 1)  # (bs, 3, 1)
    r = torch.stack([r1, r2, r3, r4, r5, r6, r7, r8, r9], dim=1).reshape(bs, 9, 1)  # (bs, 9, 1)
    ...
    B[:, 3:6, 0] = e3.reshape(bs, 3)
    B[:, 6:15, 1:4] = torch.cat([skew(r_row1), skew(r_row2), skew(r_row3)], dim=1)  # (bs, 9, 3)
...
def E_func(x):
    ...
    E[:, 3:6, 0] = e3.reshape(bs, 3)
    E[:, 6:9, 1:4] = np.sqrt(2) * torch.eye(3).type(x.type()).repeat(bs, 1, 1)
    return E
...
def Ebot_func(x):
    ...
    Ebot[:, 0:5, 0:5] = torch.eye(5).type(x.type()).repeat(bs, 1, 1)
    return Ebot
...
def D_func(x):
    ...
    R_vec = torch.tensor([0.15, 0.05, 0.05, 0.05])
