...
detach_epoch = 5  # Number of epochs to detach the computation graph
RCCM_start_epoch = 10  # Number of epochs to start RCCM training
_lambda_fixed = 0.5  # Convergence rate: lambda
w_ub = 5  # Upper bound of the eigenvalue of the dual metric
...
task = 'quad_noise' #'quad_torque'  # Name of the model
log = 'log_quad_noise_RCCM_final_normalized' #'log_quad_RCCM_torque'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    # R2 (Condition (16)) (upper-left block)
    ...
    for j in range(num_dim_noise):
        C3_inner = weighted_gradients(W, B_w[:,:,j].unsqueeze(-1), x) - (S_Bw[:,:,:,j].matmul(W) + W.matmul(S_Bw[:,:,:,j].transpose(1,2)))
        C3 = _Ebot.transpose(1,2).matmul(C3_inner).matmul(_Ebot)
        C3_inners.append(C3_inner)
        C3s.append(C3)
    ...
    if epoch < RCCM_start_epoch: 
        loss = loss_CCM + loss_w_ub + loss_C1 + loss_C2 + loss_C3 + loss_RCCM1 + loss_alpha + loss_miu
    else:
