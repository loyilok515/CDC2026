...
log = 'log_quad_delay_lambda_RCCM'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    _lambda = _lambda_nn #  _lambda = _lambda_fixed
...
def trainval(X, bs=bs, train=True, acc=False, detach=False): # trainval a set of x
    ...
    for b in _iter:
        ...
        loss, p1, p2, l3, r4, r5 = forward(x, xref, uref, w, verbose=True if not train else False, acc=acc, detach=detach)
...
for epoch in range(0, epochs):
    ...
    loss, _, _, _, _, _ = trainval(X_tr, train=True, acc=False, detach=True if epoch < detach_epoch else False)
    print("Training loss: ", loss)
    loss, p1, p2, l3, r4, r5 = trainval(X_te, train=False, acc=True, detach=False)
