...
def RK4(controller, trajectory_generator, UDE_activated, SO3_index, f, B, B_w, xinit, t_max, dt, w_MIN, w_MAX):
    ...
    for i in range(len(t)):
        ...
        # noise += np.array([0.2, -0.3, 0.1, 0.02, -0.03, 0.01]).reshape(-1,1) # Constant noise
        # noise += (w_MAX-w_MIN) * np.random.rand(w_MIN.shape[0], 1) + w_MIN # Stochastic noise
        # noise += np.array([0.2, -0.3, 0.1, 0.0, 0.0, 0.0]).reshape(-1,1)
