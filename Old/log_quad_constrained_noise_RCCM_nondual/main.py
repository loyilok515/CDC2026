...
w_ub = 10  # Upper bound of the eigenvalue of the dual metric  ### 10
w_lb = 0.1  # Lower bound of the eigenvalue of the dual metric
alpha_lb = 0.75  # Lower bound of the alpha parameter in RCCM
epsilon = _lambda_fixed * 0.1  # Ensure definiteness of LMI

# Configuration variables
task = 'quad_noise'  # Name of the model
log = 'log_quad_constrained_noise_RCCM_nondual'  # Path to a directory for storing the training log
...
def forward(x, xref, uref, w, verbose=False, acc=False, detach=False):
    ...
    if epoch < RCCM_start_epoch: 
        loss = loss_w_ub + loss_C1 + loss_C2 + loss_C2w
    else:
        loss = loss_w_ub + loss_C1 + loss_C2 + loss_C2w + loss_RCCM1 + loss_RCCM2 + loss_alpha + loss_miu
